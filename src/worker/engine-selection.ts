...
export function describeEnvironmentWorkerEngines(facts: EngineHostFacts): readonly EngineConfiguration[] {
  ...
  if (codexBinary !== undefined) {
    configurations.push({
      ...
      args: ['--strict-config'],
      sandbox: environment['SPROUT_ENV_PLATFORM'] === 'container' ? 'danger-full-access' : 'read-only',
    });
  }

  const piBinary = resolveEngineBinary('pi', facts, { preferWindowsExecutable: true });
  if (piBinary !== undefined) {
    const sessionDirectory = environment['SPROUT_PI_SESSION_DIR'];
    configurations.push({
      engine: 'pi',
      binaryPath: piBinary,
 ...
 *
 * This is the only place that reads `process.env` and `process.platform` for
 * engine selection, and the only place that shells out to find an engine CLI.
 */
...
/** An environment's override of one engine binary, e.g. `SPROUT_PI_BIN`. */
function resolveEngineBinary(
  command: string,
  facts: EngineHostFacts,
  options: { readonly preferWindowsExecutable: boolean },
): string | undefined {
  const override = facts.environment[`SPROUT_${command.toUpperCase()}_BIN`];
