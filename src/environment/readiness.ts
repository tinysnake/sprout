...
export function readinessRequirements(options: readonly { readonly engine: string; readonly workModel: string; readonly id?: string; readonly configurationVersion?: number }[], revisionInputs: readonly { readonly id: string; readonly configurationVersion?: number }[] = []): ReadinessRequirementScope {
  ...
  for (const option of options) {
    ...
    const models = mapping[option.engine] ?? (mapping[option.engine] = []);
    if (option.workModel && !models.includes(option.workModel)) models.push(option.workModel);
  }
  ...
  return {
    ...
    engines: input.engines.map((engine) => {
      ...
      return {
        ...
        ...(engine.targetModels !== undefined ? { targetModels: [...engine.targetModels] } : {}),
        ...(engine.requirementRevision !== undefined ? { requirementRevision: engine.requirementRevision } : {}),
