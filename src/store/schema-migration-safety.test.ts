...
import { DatabaseSync } from 'node:sqlite';import { CURRENT_SCHEMA_VERSION, SUPPORTED_SCHEMA_RANGE, SchemaTooNewError, SchemaTooOldError, MigrationSafetyCopyError, SchemaMigrationError, getSchemaVersion, sanitizePath, defaultSafetyCopyPath, type MigrationStep } from './schema.ts';

import { SqliteStore } from './db.ts';import { SqliteRunStore } from '../run/sqlite-store.ts';
...
test('safety copy creation failure blocks forward migration and leaves database unmodified', async () => {
  await withTempDir(async (dir) => {
    ...
    assert.equal(thrownError.toVersion, CURRENT_SCHEMA_VERSION);
