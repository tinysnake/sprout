...
export type ActiveTab = 'attention' | 'projects' | 'tasks' | 'environments' | 'agents' | 'usage' | 'onboarding';
...
class StateManager {
  ...
  public setActiveTab(tab: ActiveTab) {
    ...
    this.notify(`Navigated to ${tab} tab`);
  }
  ...
  public loadScenarioPreset(preset: string) {
    switch (preset) {
      case 'attention':
        this.state.activeTab = 'attention';
        this.notify('Loaded Scenario: Attention Hub & Action Required items');
        break;
      case 'active-task':
      ...
      case 'first-run-onboarding':
        ...
        this.notify('Loaded Scenario: First-Run Onboarding & Template Project Wizard');
        break;
      default:
        break;
    }
