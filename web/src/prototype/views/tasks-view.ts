...
export function renderTasksView(state: PrototypeState): HTMLElement {
  ...
  tasksHeader.innerHTML = `
    <div class="card-header">
      <div>
        <h2 style="font-size: 17px; font-weight: 700; display: flex; align-items: center; gap: 8px;">
          <span>🎯</span> Tasks & Lifecycle Control
        </h2>
    ...
    <div style="display: flex; gap: 6px; overflow-x: auto; padding-bottom: 2px;">
      ${state.tasks
        .map(
          (t) => `
        <button class="btn btn-sm task-select-btn ${state.selectedTaskId === t.id ? 'btn-primary' : 'btn-secondary'}" data-task="${t.id}" style="white-space: nowrap;">
          ${
            t.lifecycle === 'awaiting validation'
              ...
              ? '💡'
              : '✅'
          } #${t.id.replace('task-', '')}: ${t.currentVersion.title.slice(0, 20)}...
        </button>
      `
        )
        .join('')}
    ...
    claimCard.innerHTML = `
      ...
      <div style="display: flex; gap: 8px; justify-content: flex-end; flex-wrap: wrap; margin-top: 6px;">
        <button class="btn btn-warning require-correction-btn">
          ⚠️ Require Correction
        </button>
        <button class="btn btn-primary accept-claim-btn">
          ✅ Accept & Authorize Safe Task End
        </button>
      </div>
    `;
  ...
  runsCard.innerHTML = `
    <div class="card-header">
      <span class="card-title">⏱️ Nested Agent Runs (${selectedTask.runs.length})</span>
