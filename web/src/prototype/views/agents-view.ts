  ...
  agentsHeader.innerHTML = `
    <div class="card-header">
      <div>
        <h2 style="font-size: 17px; font-weight: 700; display: flex; align-items: center; gap: 8px;">
          <span>🤖</span> Agents & Work Option Preferences
        </h2>
    ...
    <div style="display: flex; gap: 6px; overflow-x: auto; padding-bottom: 2px;">
      ${state.agents
        .map(
          (a) => `
        <button class="btn btn-sm agent-tab-btn ${state.selectedAgentId === a.id ? 'btn-primary' : 'btn-secondary'}" data-agent="${a.id}">
          <span>${a.avatar}</span> ${a.displayName}
        </button>
      `
        )
        .join('')}
  ...
  detailCard.innerHTML = `
    <div class="card-header">
      <div style="display: flex; align-items: center; gap: 10px;">
        <span style="font-size: 28px;">${selectedAgent.avatar}</span>
        <div>
          ...
          .map((env) => {
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-subtle); padding-bottom: 4px;">
              ...
              <span class="status-pill ${hasCompatibleOption ? 'green' : 'yellow'}" style="font-size: 11px;">
                ${hasCompatibleOption ? '✅ Compatible (First option available)' : '⚠️ Requires Engine Login / Setup'}
              </span>
            </div>
          `;
          })
