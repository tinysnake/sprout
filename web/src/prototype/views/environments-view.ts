    ...
    <div class="card-header">
      <div>
        <h2 style="font-size: 17px; font-weight: 700; display: flex; align-items: center; gap: 8px;">
          <span>🖥️</span> Environments & Host Workers
        </h2>
    ...
    <div style="display: flex; flex-direction: column; gap: 10px;">
      ...
      <div style="background: var(--bg-surface-elevated); padding: 12px; border-radius: var(--radius-sm); font-size: 12px; display: flex; flex-direction: column; gap: 6px;">
        ...
        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
          <span>File R/W: ${selectedEnv.capabilityPermissions.fileReadWrite ? '✅ Granted' : '❌ Refused'}</span>
          <span>Process Execution: ${selectedEnv.capabilityPermissions.processExecution ? '✅ Granted' : '❌ Refused'}</span>
          <span>Network: ${selectedEnv.capabilityPermissions.networkAccess ? '✅ Granted' : '❌ Refused'}</span>
          <span>GUI Automation: ${selectedEnv.capabilityPermissions.guiAutomation ? '✅ Granted' : '❌ Refused'}</span>
        </div>
      </div>
    ...
    <div style="display: flex; gap: 8px; flex-wrap: wrap; justify-content: flex-end; border-top: 1px solid var(--border-subtle); padding-top: 12px;">
      ...
      ${
        selectedEnv.workSafety === 'recovery'
          ? `<button class="btn btn-danger btn-sm force-release-btn">🚨 Emergency Force Release</button>`
          : ''
      }
    </div>
  `;
